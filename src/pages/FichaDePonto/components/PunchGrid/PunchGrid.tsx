...
export function PunchGrid({ days, funcionarioId, funcionarioNome, turnoId, tzOffset, onReload, reloading, canEdit, onToggleBloqueio }: PunchGridProps) {
  ...
  const handleCellOpen = useCallback((dayLabel: string, slotLabel: string, row: DayRow, e: React.MouseEvent) => {
    if (row.bloqueado) return; // Dia bloqueado — não permite abrir menu de edição
    e.stopPropagation();
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      ...
      // Não mostra highlight de drop em dias bloqueados
      let validHit = hit;
      if (hit) {
        const [hy, hm, hd] = hit.key.split('-').map(Number);
        const targetRow = daysRef.current.find(r => r.year === hy && r.month === hm && r.day === hd);
        if (targetRow?.bloqueado) validHit = null;
      }
      setDragOverKey(validHit ? validHit.key : null);
    };
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (!targetRow || targetRow.bloqueado) return; // Dia destino bloqueado — não permite soltar
  ...
  const handleCellPointerDown = useCallback((row: DayRow, slotIndex: number, e: React.PointerEvent) => {
    if (row.bloqueado) return; // Dia bloqueado — não permite mover batidas
