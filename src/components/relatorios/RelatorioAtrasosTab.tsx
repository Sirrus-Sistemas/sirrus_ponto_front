        ...
        for (const dia of espelho.dias) {
          if (
            ...
            result.push({
              ...
              minutosTrabalhados: dia.minutos_trabalhados,
              minutosPrevistos: dia.minutos_previstos,
              saldoMinutos: dia.saldo_minutos,
            })
          }
        }
      } catch {
        ...
        {rows !== null ? (
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr><td colSpan={8} className={styles.tableEmpty}>Nenhum atraso encontrado no período.</td></tr>
                ) : rows.map((r, i) => (
                  <tr key={`${r.funcionario.id}-${r.data}-${i}`}>
                    ...
                    <td>{r.diaSemana}</td>
                    <td>{formatMinutos(r.minutosTrabalhados)}</td>
                    <td>{r.minutosPrevistos !== null ? formatMinutos(r.minutosPrevistos) : '—'}</td>
                    <td style={{ color: '#b91c1c' }}>{formatMinutos(r.saldoMinutos)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {rows && rows.length > 0 ? (
        <div className={styles.printSection}>
          <RelatorioPrintLayout
            reportNum={3}
            reportTitle={`Atrasos — ${labelMes(mes)} de ${ano}`}
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={`${r.funcionario.id}-${r.data}-${i}`}>
                    ...
                    <td>{fmtDate(r.data)}</td>
                    <td>{r.diaSemana}</td>
                    <td>{minToHHMM(r.minutosTrabalhados)}</td>
                    <td>{r.minutosPrevistos !== null ? minToHHMM(r.minutosPrevistos) : ''}</td>
                    <td>{minToHHMM(r.saldoMinutos)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
